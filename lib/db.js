...
// Hosted platforms give you a read-only filesystem (Vercel) or a writable
// scratch dir (/tmp). DB_FILE lets you point at whichever you have.
...
function persist(immediate) {
  if (immediate) {
    try {
      ...
      // Read-only filesystem: keep serving from memory rather than 500ing.
      if (!warnedReadOnly) {
        warnedReadOnly = true;
        console.warn(`[db] no puedo escribir en ${FILE} (${err.code}). Los datos viven solo en memoria.`);
      }
    }
    return;
  }
